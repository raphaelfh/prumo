...
- Every `.py` check reads `--repo-root PATH`. `--scope GLOB` only where a narrowed scan is meaningful (today `check_legacy_concepts.py`, the one gate `run_all.sh --scope` forwards to). Most also accept `--jsonl-out PATH` + `--emit-telemetry PATH` (see [Harness contract](#harness-contract)); five accept neither: `check_diff_attribute_copy.py`, `check_file_size.py`, `check_response_descriptions.py`, `check_retired_symbols.py`, `check_ui_primitives.py`. `.sh` checks are thin wrappers when an existing script already enforces the invariant.
...
| `check_response_descriptions.py` | Every `responses=` entry under `backend/app/` (route decorator, `APIRouter`, `include_router`) is a dict literal with an explicit `"description"`. FastAPI's default is `http.HTTPStatus(code).phrase`, which differs across Python versions (422: 3.11 vs 3.13), so the committed contract would depend on who regenerated it. Non-literal `responses=` fails as unverifiable. Absolute: no baseline. |
...
| `check_file_size.py` | Ratchet, not a ceiling: a baselined oversized file may not grow and no new file may cross the soft limit (800 lines). Shrinking always passes and lets the baseline tighten. |
