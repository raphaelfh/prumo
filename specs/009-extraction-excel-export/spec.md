...
- **FR-018**: The "Articles to export" selector defines the universe of candidate articles. `Current list` resolves to the articles visible on the Data Extraction page given the user's current filters/search; `Selected only` resolves to articles whose checkbox is ticked in the Article-Extraction table. The selected universe is THEN intersected with the mode's stage-eligibility rule (FR-013 / FR-014 / FR-015). If the intersection is empty, the Export button is disabled with the inline reason.
...
- **FR-022**: Generation MUST happen server-side, reading from the database with project-membership authorisation. The endpoint MUST return either (a) the `.xlsx` bytes directly for small payloads (≤ 5 MB or expected generation ≤ 10 s), or (b) a signed download URL for payloads beyond that threshold (following the async pattern already used by `articles_export_service.run_export_async`).
- **FR-023**: When the export runs in async mode, the dialog MUST close after the job is dispatched and the result MUST be delivered through the in-app notification center / toast ("Your extraction export is ready — Download" with a link). This mirrors the existing async export pattern used for articles and keeps the user unblocked.
...
- **FR-025**: The export operation MUST be logged in the project's audit trail with: actor, mode, target reviewer (if applicable), template_id, article count, generated_at, the "Anonymize reviewer names" toggle state (default: off / names visible), the "Include AI metadata sheet" toggle state (default: off), and the chosen article-scope (`Current list` vs `Selected only` with the count).
...
These build on the Articles-Export dialog baseline; each is in V1 scope unless flagged.

- **FR-032 (V1)**: The dialog MUST show a recap of the user's choices in the live preview line (FR-027) so the user can self-verify before committing. This is the strongest defence against "I downloaded the wrong slice" errors that the current Articles dialog does not protect against.
- **FR-033 (V1)**: Async-export completion notifications MUST be persistent (the in-app notification center entry remains until the user clicks it), with the signed URL embedded — the user can leave the page and return later.
