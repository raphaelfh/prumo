...
async def seed_probast(session: AsyncSession) -> None:
    ...
    items_data = [
        ...
        ("participants", "1.2",
         "Were all inclusions and exclusions of participants appropriate?", 1),
        ("predictors", "2.1",
         "Were predictors defined and assessed in a similar way for all participants?", 2),
        ("predictors", "2.2",
         "Were predictor assessments made without knowledge of outcome data?", 3),
        ("predictors", "2.3",
        ...
        ("outcome", "3.5",
         "Was the outcome determined without knowledge of predictor information?", 9),
        ("outcome", "3.6",
         "Was the time interval between predictor assessment and outcome determination appropriate?", 10),
        ("analysis", "4.1",
         "Were there a reasonable number of participants with the outcome?", 11),
        ("analysis", "4.2",
         "Were continuous and categorical predictors handled appropriately?", 12),
        ("analysis", "4.3",
         "Were all enrolled participants included in the analysis?", 13),
        ("analysis", "4.4",
         "Were participants with missing data handled appropriately?", 14),
        ("analysis", "4.5",
         "Was selection of predictors based on univariable analysis avoided?", 15),
        ("analysis", "4.6",
         "Were complexities in the data (e.g. censoring, competing risks, sampling) accounted for appropriately?", 16),
        ("analysis", "4.7",
         "Were relevant model performance measures evaluated appropriately?", 17),
        ("analysis", "4.8",
         "Were model overfitting and optimism in model performance accounted for?", 18),
        ("analysis", "4.9",
    ...
    for domain, code, question, sort in items_data:
        instrument.items.append(AssessmentItem(
...
async def seed_charms(session: AsyncSession) -> None:
    ...
    # ---- Template ----
    session.add(ExtractionTemplateGlobal(
    ...
    # ---- Fields ----
    def _f(eid, name, label, desc, ftype, sort, *, required=True, allowed=None, unit=None, llm=None):
    ...
    fields = [
        # --- prediction_models (2) ---
        _f(_ET_PREDICTION_MODELS, "model_name", "Model Name",
        ...
        _f(_ET_PREDICTION_MODELS, "modelling_method", "Modelling Method",
           "Statistical modelling method used (CHARMS 7.1)", "select", 1,
           allowed=["Logistic regression", "Cox regression", "Linear regression",
        ...
        # --- source_of_data (1) ---
        _f(_ET_SOURCE_OF_DATA, "data_source", "Source of Data",
           "Data source used in the study (CHARMS 1.1)", "select", 0,
           allowed=["Prospective cohort", "Retrospective cohort", "Case-control",
                    "Case series", "RCT", "Registry", "No information"],
           llm="Extract the source of data used in the study."),

        # --- participants (12) ---
        _f(_ET_PARTICIPANTS, "recruitment_method", "Recruitment Method",
           "Method used to recruit participants (CHARMS 2.1)", "select", 0,
           allowed=["Consecutive patients", "Random sampling",
        ...
        _f(_ET_PARTICIPANTS, "study_sites_regions", "Study Sites (Regions)",
        ...
        _f(_ET_PARTICIPANTS, "native_valve_endocarditis", "Native Valve Endocarditis",
        ...
        _f(_ET_PARTICIPANTS, "valve_affected", "Valve Affected",
           "Affected valves (CHARMS 2.8.3)", "text", 10,
           llm="Extract information about which valves were affected."),
        _f(_ET_PARTICIPANTS, "characteristic_4", "Characteristic 4",
        ...
        _f(_ET_PARTICIPANTS, "characteristic_5", "Characteristic 5",
        ...
        _f(_ET_OUTCOME, "outcome", "Outcome",
        ...
        _f(_ET_OUTCOME, "outcome_definition", "Outcome Definition",
           "Precise definition of the outcome (CHARMS 3.2)", "text", 1,
           llm="Extract the precise definition of the outcome variable."),
        _f(_ET_OUTCOME, "same_outcome_definition", "Same Outcome Definition for All Participants",
        ...
        _f(_ET_OUTCOME, "type_of_outcome", "Type of Outcome",
        ...
        _f(_ET_OUTCOME, "outcome_assessed_blinded", "Outcome Assessed Blinded to Predictors",
        ...
        _f(_ET_OUTCOME, "predictors_part_of_outcome", "Predictors Part of Outcome",
        ...
        _f(_ET_OUTCOME, "time_of_outcome", "Time of Outcome Occurrence",
        ...
        _f(_ET_CANDIDATE_PRED, "number_of_candidates", "Number of Candidate Predictors",
           "Total candidate predictors assessed (CHARMS 4.1)", "number", 0, unit="predictors",
           llm="Extract the total number of candidate predictors assessed for inclusion."),
        _f(_ET_CANDIDATE_PRED, "type_of_predictors", "Type of Predictors",
           "Type of candidate predictors (CHARMS 4.2)", "text", 1,
           llm="Extract the type of candidate predictors assessed."),
        _f(_ET_CANDIDATE_PRED, "timing_of_measurement", "Timing of Predictors Measurement",
           "When predictors were measured (CHARMS 4.3)", "select", 2,
           allowed=["Pre-operative", "Post-operative", "At baseline",
                    "During follow-up", "No information"],
           llm="Extract when the candidate predictors were measured relative to the outcome."),
        _f(_ET_CANDIDATE_PRED, "predictors_definition_similar",
        ...
        _f(_ET_CANDIDATE_PRED, "predictors_assessed_blinded",
        ...
        _f(_ET_CANDIDATE_PRED, "handling_continuous", "Handling of Continuous Predictors",
           "How continuous predictors were handled (CHARMS 4.6)", "select", 6,
           allowed=["Kept continuous", "Categorized",
                    "Restricted cubic spline function", "No information"],
           llm="Extract how continuous predictors were handled in the model."),

        # --- sample_size (3) ---
        _f(_ET_SAMPLE_SIZE, "number_of_participants", "Number of Participants",
           "Total participants in the study (CHARMS 5.1)", "number", 0, unit="participants",
           llm="Extract the total number of participants included in model development."),
        _f(_ET_SAMPLE_SIZE, "number_of_events", "Number of Outcomes/Events",
           "Observed outcome events (CHARMS 5.2)", "number", 1, unit="events",
           llm="Extract the number of outcome events that occurred in the study sample."),
        _f(_ET_SAMPLE_SIZE, "epv_epp", "Events Per Variable (EPV) or Per Parameter (EPP)",
           "EPV or EPP ratio (CHARMS 5.3)", "number", 2, unit="events/variable",
           llm="Extract the EPV or EPP ratio, indicating sample size adequacy for model development."),

        # --- missing_data (2) ---
        _f(_ET_MISSING_DATA, "participants_with_missing", "Participants with Any Missing Value",
        ...
        _f(_ET_MODEL_DEV, "selection_method_multivariable",
           ...
           allowed=["Backward elimination", "Forward selection", "Stepwise",
        ...
        _f(_ET_MODEL_DEV, "shrinkage", "Shrinkage of Predictor Weights",
        ...
        # --- final_predictors (2) ---
        _f(_ET_FINAL_PREDICTORS, "predictor_name", "Predictor Name",
        ...
        _f(_ET_FINAL_PREDICTORS, "predictor_weight", "Predictor Weight",
        ...
        _f(_ET_MODEL_PERF, "calibration_plot", "Calibration Plot",
        ...
        _f(_ET_MODEL_PERF, "calibration_slope", "Calibration Slope",
        ...
        _f(_ET_MODEL_PERF, "hosmer_lemeshow_p_value", "Hosmer-Lemeshow Test (p-value)",
           "Hosmer-Lemeshow p-value", "text", 8,
           llm="Extract the p-value from Hosmer-Lemeshow test if reported."),
        _f(_ET_MODEL_PERF, "calibration_other", "Other Calibration Measures",
           "Other calibration measures reported? (CHARMS 8.1.5)", "select", 9,
           allowed=_YES_NO_NI,
           llm="Determine if other calibration measures were reported."),
        _f(_ET_MODEL_PERF, "calibration_other_specify", "Other Calibration Measures (Specify)",
        ...
        _f(_ET_MODEL_PERF, "log_rank_test_p_value", "Log-Rank Test (p-value)",
           "Log-rank test p-value", "text", 19,
           llm="Extract the p-value from log-rank test if reported."),
        _f(_ET_MODEL_PERF, "risk_group_curves", "Risk Group Curves (if survival analysis)",
           "Risk group curves presented? (CHARMS 8.2.5)", "select", 20,
           allowed=_YES_NO_NOTAPP_NI,
           llm="If survival analysis, determine if risk group curves were presented."),
        _f(_ET_MODEL_PERF, "discrimination_other", "Other Discrimination Measures",
        ...
        _f(_ET_MODEL_PERF, "discrimination_other_specify",
        ...
        _f(_ET_MODEL_PERF, "r_squared", "R-squared",
        ...
        _f(_ET_MODEL_PERF, "overall_other_specify", "Other Overall Measures (Specify)",
        ...
        # --- model_results (4) ---
        _f(_ET_MODEL_RESULTS, "formula_or_score", "Final Model Formula or Score",
        ...
        _f(_ET_MODEL_RESULTS, "predictor_weights_included",
        ...
        _f(_ET_MODEL_RESULTS, "intercept_included",
        ...
        _f(_ET_MODEL_INTERP, "interpretation", "Interpretation of Presented Model",
        ...
        _f(_ET_MODEL_OBS, "data_extraction_process", "Data Extraction Process",
           "Status of data extraction (CHARMS 12.1)", "select", 0,
           allowed=["All information has been successfully registered",
                    "Some information missing", "Extraction incomplete"],
           llm="Record the status of data extraction for this model."),
        _f(_ET_MODEL_OBS, "additional_information", "Additional Information",
