...
async def load_suggestions(
    ...
    *,
    ...
    proposal_stmt = (
        ...
        .order_by(ExtractionProposalRecord.created_at.desc(), ExtractionProposalRecord.id.desc())
    )
    ...
    context = await suggestion_reveal_context(db, deduped, caller_id=caller_id)
    for p in deduped:
        evidence_list = [
            EvidenceResponse(
...
async def get_suggestion_history(
    ...
    *,
    ...
    ``ran_by_user_id``/``ran_by_name`` in immutable generation details.
    ...
    proposals = (
        (
            await db.execute(
                ...
                .order_by(
                    ExtractionProposalRecord.created_at.desc(), ExtractionProposalRecord.id.desc()
                )
                .limit(limit)
            )
        )
    ...
    context = await suggestion_reveal_context(db, proposals, caller_id=caller_id)
    for p in proposals:
        evidence_list = [
            EvidenceResponse(
                proposal_record_id=p.id,
