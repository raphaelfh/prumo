...
async def test_create_proposal_awaits_reviewer_role_gate() -> None:
    """``source='human'`` is the only value this endpoint does not reject
    itself — human writes are turned away one layer down, in
    ``record_proposal`` (mocked here), so this isolates the role gate."""
    run_id, project_id, caller = uuid4(), uuid4(), uuid4()
    record = SimpleNamespace(
        id=uuid4(),
        run_id=run_id,
...
@pytest.mark.parametrize("source", ["ai", "system"])
@pytest.mark.asyncio
async def test_create_proposal_rejects_server_generated_sources(source: str) -> None:
    """Both server-generated sources are refused before any service call.

    'ai' rows are written in-process by ``SectionExtractionService``; blind
    peers read them unattributed, so a caller-authored one is a forged model
    suggestion. 'system' rows seed peers' form baselines. The integration
    coverage goes through ASGI, whose handler lines do not register on
    coverage — this calls the coroutine directly so the guard is covered.
    """
    run_id, project_id, caller = uuid4(), uuid4(), uuid4()
    body = CreateProposalRequest(
        instance_id=uuid4(),
    ...
    assert f"source='{source}'" in str(exc.value.detail)
    assert "server-generated" in str(exc.value.detail)
    # The role gate still ran first, and nothing was written.
    gate.assert_awaited_once_with(db, project_id, caller)
