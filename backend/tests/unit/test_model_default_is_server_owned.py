...
C1a deleted the field; the surviving schemas set `extra='forbid'` and REJECT
...
later than the retired `ModelExtractionRequest`, on purpose: they round-trip
through Celery, and a job queued by a pre-C1a web process still carried the
dropped key — under `forbid` that rebuild dies terminally, with no retry. The
pre-C1a era reached prod on 2026-08-11 and the queue drains in minutes, so by
2026-08-16 the straddle window was provably empty
(`app/schemas/extraction.py` records the full sequencing). Trees B6 retired
`ModelExtractionRequest` with its route; the two schemas a client can still
reach keep their guards below.
