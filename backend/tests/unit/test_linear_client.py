...
async def test_graphql_raises_on_errors(client: LinearClient) -> None:
    ...
    with (
        patch("httpx.AsyncClient.post", new=AsyncMock(return_value=_resp(body))),
        pytest.raises(LinearError),
    ):
...
async def test_resolve_labels_creates_missing(client: LinearClient) -> None:
    ...
    with patch(
        "httpx.AsyncClient.post", new=AsyncMock(side_effect=[_resp(team_labels), _resp(created)])
    ):
...
async def test_upload_file_requests_url_then_puts(client: LinearClient) -> None:
    up = {
        "data": {
            "fileUpload": {
                "success": True,
                "uploadFile": {
                    "uploadUrl": "https://upload/put",
                    "assetUrl": "https://asset/x.webp",
    ...
    with patch("httpx.AsyncClient.post", new=post), patch("httpx.AsyncClient.put", new=put):
        asset = await client.upload_file(
            data=b"bytes", content_type="image/webp", filename="x.webp"
        )
