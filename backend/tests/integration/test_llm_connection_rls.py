"""RLS + privilege probes for ``llm_connections`` (migration 0072).

The table stores Fernet-encrypted connection API keys, so its posture is
stricter than the config tables': API-only, nothing readable from
PostgREST. 0072 ships two layers and this file pins both:
...
async def test_no_privilege_granted(db_session: AsyncSession, role: str, privilege: str) -> None:
    ...
    assert granted is False, (
        f"{role} holds {privilege} on {_TABLE} — the connection-secrets table "
        "must be API-only (migration 0072 REVOKE did not take effect)"
    )


@pytest.mark.asyncio
async def test_select_denied_by_missing_grant(db_session: AsyncSession) -> None:
    """Even a project manager's JWT must not read connection rows: the
