...
uuid, a non-int or negative position, a non-numeric rank) is the caller's
...
@pytest.mark.parametrize(
    "values",
    [["0.5", "not-a-uuid"], ["not-a-rank", str(uuid4())]],
    ids=["bad-uuid", "bad-rank"],
)
...
@pytest.mark.parametrize(
    "values",
    [["x", 0, 0], [1, 0, -5], [-1, 0, 0]],
    ids=["non-int", "negative-offset", "negative-page"],
)
