 ...
 * - Suporta schemas complexos com aninhamento (ideal para schemas enriquecidos)
 * 
...
/**
 * Configuração de timeout e limites por modelo
 */
export interface ExtractionConfig {
  ...
  llm: {
    timeout: {
      base: number; // ms
      gpt5?: number; // ms para gpt-5
      warningThreshold: number; // 0-1 (ex: 0.8 = 80%)
    };
    maxTextLength: {
      base: number;
      gpt5?: number; // Tamanho máximo para gpt-5
    };
  };
}

/**
 * Helper para calcular timeout e limites baseado no modelo
 */
class ConfigHelpers {
  static getTimeoutMs(model: SupportedModel, config: ExtractionConfig): number {
    return model === "gpt-5" && config.llm.timeout.gpt5
      ? config.llm.timeout.gpt5
      : config.llm.timeout.base;
  }

  static getMaxTextLength(model: SupportedModel, config: ExtractionConfig): number {
    return model === "gpt-5" && config.llm.maxTextLength.gpt5
      ? config.llm.maxTextLength.gpt5
      : config.llm.maxTextLength.base;
  }

  static getTimeoutWarningMs(model: SupportedModel, config: ExtractionConfig): number {
    const timeoutMs = ConfigHelpers.getTimeoutMs(model, config);
    return Math.floor(timeoutMs * config.llm.timeout.warningThreshold);
  }
}
 ...
 */
export interface LangchainExtractionResult {
 ...
 */
export class LangchainExtractor {
  private static readonly DEFAULT_MODEL: SupportedModel = "gpt-4o-mini";
  ...
  async extract(
    ...
    prompt: string,
    options: LangchainExtractionOptions = {},
  ): Promise<LangchainExtractionResult> {
    ...
    try {
      ...
      const modelName = (options.model || LangchainExtractor.DEFAULT_MODEL) as SupportedModel;
      ...
      const LLM_TIMEOUT_MS = ConfigHelpers.getTimeoutMs(modelName, this.config);
      const MAX_TEXT_LENGTH = ConfigHelpers.getMaxTextLength(modelName, this.config);
      const TIMEOUT_WARNING_MS = ConfigHelpers.getTimeoutWarningMs(modelName, this.config);
          ...
          try {
            ...
            timeoutWarningTimer = setTimeout(() => {
              ...
              this.logger.warn("LLM extraction approaching timeout", {
                ...
                warningThreshold: `${(this.config.llm.timeout.warningThreshold * 100).toFixed(0)}%`,
            ...
            this.logger.error("Agent invocation failed", err as Error, {
              ...
              errorStack,
              model: modelName,
              errorType: isTimeout ? "timeout" : isRateLimit ? "rate_limit" : isValidationError ? "validation" : isInvalidResponse ? "invalid_response" : "unknown",
            });
            ...
            // Erro de resposta inválida da API (pode ser problema transitório ou do modelo)
