...
db-lint-migrations: ## Lint migrations with Squawk (catches NOT NULL without default, missing FK indexes, dangerous drops)
	...
	@# Skip baseline_v1.sql: autogenerated pg_dump squash; squawk's parser
	@# can't handle pg_trgm operator class syntax (USING gin (col gin_trgm_ops))
	@# and the file is immutable history — re-applying it to a healthy DB
	@# is a no-op so the lint adds no signal.
	@MIGRATIONS=$$(find $(BACKEND_DIR)/alembic/versions -maxdepth 1 -name "*.sql" -o -name "*.py" 2>/dev/null | grep -v archive | grep -v baseline_v1.sql); \
