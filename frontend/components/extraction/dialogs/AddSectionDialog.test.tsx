...
describe('AddSectionDialog — root mode (unchanged B-7 contract)', () => {
  ...
  it('renders the technical-name hints through copy', () => {
    // Asserted through t() so the indirection is what is pinned — a
    // hardcoded revert of these strings would not satisfy it.
    renderDialog({kind: 'root'});
    expect(screen.getByText(t('extraction', 'sectionNameLabel'))).toBeInTheDocument();
    expect(screen.getByText(t('extraction', 'sectionNameHint'), {exact: false})).toBeInTheDocument();
    expect(
      screen.getByText(t('extraction', 'sectionNameAutoGenerated'), {exact: false}),
    ).toBeInTheDocument();
    expect(screen.getByText(t('extraction', 'sectionLabelHint'))).toBeInTheDocument();
  });
});
