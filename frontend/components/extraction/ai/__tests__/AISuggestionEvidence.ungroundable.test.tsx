...
describe('AISuggestionEvidence ungroundable', () => {
  ...
  it('applies the cautionary warning treatment to the ungroundable badge', () => {
    render(<AISuggestionEvidence evidence={ev} />, {wrapper: Wrapper});
    // The badge must keep its warning (caution) styling — a regression that
    // drops it to a neutral/entailed look should fail here, not just slip
    // past the copy assertion above.
    expect(screen.getByText(/verify manually/i).className).toMatch(/text-warning/);
  });
});
