...
/** Shared immutable facts. Only the legacy dialog supplies its existing article-text slot. */
export function GenerationDetailsContent({provenance, historicalInputAvailable = false, articleText}: {
  provenance?: RunProvenance;
  historicalInputAvailable?: boolean;
  articleText?: ReactNode;
}) {
  const {showPeerIdentity} = useRunEditability();
  if (!provenance) return <p className="text-xs text-muted-foreground">{t('extraction', 'reviewGenerationUnavailable')}</p>;
      ...
      <section className="space-y-2.5">
        ...
        <RecipeStep index={3}>
          <div className="rounded-md border border-ai/30 bg-ai/10 px-3 py-2">
            ...
            {articleText ?? <p className="mt-1 text-xs text-muted-foreground">{t('extraction', historicalInputAvailable ? 'reviewHistoricalInputRecorded' : 'reviewHistoricalInputUnavailable')}</p>}
          </div>
        </RecipeStep>
