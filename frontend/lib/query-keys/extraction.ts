/**
 * TanStack Query key factory for extraction reads outside the run view. The
 * run view (stage, instances, proposals, decisions) is `runsKeys.detail(runId)`
 * in `hooks/runs/types.ts`, read by `useRun`; invalidate that key after a
 * run changes.
 */
