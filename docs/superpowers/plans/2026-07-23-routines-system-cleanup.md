...
| 2026-07-24 ~13:20 | — | **Two design facts the transcripts surfaced.** (1) The probation ceiling blocked `bug-fix` from fixing a real, high-value security bug it correctly found; it improvised a private notification to surface it, which is *outside* the "a PR or nothing, never a side-channel" contract but was the right call given no sanctioned channel existed. Open question for the maintainer: allow security/BOLA findings to open a PR even during probation, or keep the ceiling and accept the side-channel for them. (2) `gh` is **not installed** in the routine sandbox (issue #186 already noted this); both routines adapted to GitHub **MCP** tools for the read/dedup steps, but neither reached the write path, so `gh pr create` in the prompts is **unexercised** — the first run that actually wants to open a PR may need to fall back to MCP too. |
