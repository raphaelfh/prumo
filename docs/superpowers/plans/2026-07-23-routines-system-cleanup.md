...
| 2026-07-24 01:3x | 5 | Backlog **43 → 12**. 31 closed with per-issue evidence, 4 fixed in this branch (#284, #333, #285, #406). See `2026-07-23-auto-found-triage.md`. |
| 2026-07-24 02:06 | 2 | Manual `run` of `pr-review` with Context7 already restored: **no artifact in 9 minutes**. This exonerates the MCP change — the configuration is not the differentiator. Four cloud sessions fired between 01:07Z and 02:06Z and none produced any artifact, so the cause is platform-side capacity (queue or run cap), not config. The revert stands on principle, not on this evidence. |
| 2026-07-24 02:16 | 3, 4 | **Verification incomplete and recorded as such.** The rewritten `cleanup` and the new `bug-fix` have never been observed completing a cloud run. Their gate is validated command by command locally — the stronger evidence — but end-to-end proof waits on the first scheduled runs: `bug-fix` Saturday 06:00 UTC, `cleanup` Tuesday 09:00 UTC. If those produce nothing either, the gate is not the cause and the investigation should start at platform capacity. |
| 2026-07-24 12:03 | 2 | ~~Definitive: the silence is platform-side, account-wide.~~ **RETRACTED — see the row below.** ~10 hours after the earlier failures, `pr-review` was triggered manually again (`cse_01HUdujQe6TuLzkgDKeKQS3S`) and produced nothing in 10+ minutes, which I read as an account-wide platform failure. That reading was **wrong**. |
| 2026-07-24 ~13:00 | 2, 3, 4 | **Correction, from the actual session transcripts (`claude.ai/code`): all three routines ran correctly. There was no platform failure.** The silence had three distinct, deterministic, *correct* causes: **`cleanup`** ran, gated, found `backend/app/utils` genuinely clean (only a 42-LOC `rate_limiter.py` + a 1-line `__init__.py`, no dead code, ruff clean) → `action=skip reason=nothing-to-clean`. A valid, healthy no-op. **`bug-fix`** ran, landed on scope 2 as predicted, and **found a real high-confidence BOLA leak** in `articles_export.get_export_status` — then correctly *withheld the fix* because BOLA is outside its probation ceiling → `action=skip reason=above-ceiling`. **`pr-review`** ran and **correctly skipped #554**: the committed `automated-pr-review.md` contract has a hard rule to skip `claude/*` branches that lack a `needs-review` label, and #554 is a `claude/*` branch with no such label; #553/#550/#549 were already reviewed (dedup). My mistake: I never read that skip rule before spending an hour and four manual triggers chasing a non-bug — the rule is in a file I had partially read. The end-to-end verification I had marked "incomplete" is now **complete and passing**: the three routines execute and produce correct outcomes. The one genuine finding (#555) is a real security fix. |
| 2026-07-24 ~13:20 | — | **Two design facts the transcripts surfaced** (both resolved in the 14:00 pass below). (1) The probation ceiling blocked `bug-fix` from fixing the real security bug it found; it improvised a side-channel notification. (2) `gh` is not installed in the sandbox (issue #186); the `gh pr create` write path in both prompts was unexercised and would have aborted the first PR-opening run. |
| 2026-07-24 14:00 | 3 | **SOTA-hardening pass — the routines are now functional end to end on paper, not just able to find things.** Three config changes via `RemoteTrigger`: **(a)** both `cleanup` and `bug-fix` prompts now open with a "GitHub access" section — `gh` is absent, route every GitHub op (list, branch, commit, PR) through GitHub MCP, `git` for local only; this fixes the guaranteed write-path abort. **(b)** `bug-fix`'s probation ceiling now permits confirmed security findings (BOLA, auth bypass, injection, secret/PII leak) to open a PR, with the side-channel explicitly forbidden. **(c)** `pr-review` bumped `claude-sonnet-4-6 → claude-sonnet-5` (model currency = burden prevention; prompt untouched). Verified via `list`: `bug-fix` opus-4-8[1m] + gh→MCP + security-ceiling; `cleanup` opus-4-8[1m] + gh→MCP; `pr-review` sonnet-5. All three unverified end to end (no session output; manual triggers currently skip) — the first scheduled run that finds something is the real test of the MCP write path. |
