...
- **R18** `ZoomControls` calls `zoomBy` / `setZoom` only; its inline clamp and ±0.25 arithmetic are deleted. Amended 2026-09-15 after Phase 2: the zoom percentage label and its preset menu are removed from the toolbar — the percentage costs bar space and the presets are reachable by stepping. Fit width is its own `IconButton` in the bar, not a menu item, and shows `aria-pressed` while the document is fitted. A `☰` `DropdownMenu` sits at the far right of the bar and holds Rotate view (moved out of the bar, canvas mode only) and Open article page (rendered only when the host app supplies a link; opens `https://doi.org/<doi>` in a new tab with `noopener,noreferrer`). The viewer package stays article-agnostic: it takes a generic link (label + href), and the app decides what that link is.
